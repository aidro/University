...
function fixapt(){
    ...
    echo -e "\n http://download.proxmox.com/debian/ceph-reef bookworm no-subscription" >> /etc/apt/sources.list.d/ceph.list
...
function terraform(){
    ...
    # Creating necessary directories for later us
...
function cephcluster(){
    ...
    pveceph pool create cephfs --add-storages
}
...
function createlxc(){
    ...
    sed -i "609i arch: amd64" "/usr/share/perl5/PVE/LXC.pm"
    pct stop $initial_id
    ...
    # Wait 5 minutes for each container to provision
    sleep 300

    # List of hosts in cluster
    hosts=$(pvecm status | grep "10\." | awk '{print $3}' | sed 's/ *(local)//')
    for host in $hosts;
    do
        for id in "${id_list[@]}";
        do
